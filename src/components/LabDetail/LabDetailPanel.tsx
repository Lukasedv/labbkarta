...
export function LabDetailPanel({ lab, allLabs, onClose, onSelectLab }: LabDetailPanelProps) {
  ...
  return (
    <>
      {/* Backdrop overlay */}
      <div
        className={`absolute inset-0 z-[999] bg-black transition-opacity duration-300 ${
      ...
      {/* Panel */}
      <div
        className={`absolute right-0 top-0 h-full w-96 max-w-[calc(100vw-4rem)] bg-white shadow-xl border-l border-gray-200 z-[1000] flex flex-col overflow-hidden transition-transform duration-300 ${
          ...
          {nearbyLabs.length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-200">
              ...
              <div className="space-y-1">
                {nearbyLabs.map(({ lab: nearby, distance }) => (
                  <button
                    key={nearby.id}
                    onClick={() => onSelectLab(nearby)}
                    className="w-full text-left px-2 py-1.5 rounded hover:bg-blue-50 transition-colors cursor-pointer border-none bg-transparent"
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className="w-2 h-2 rounded-full flex-shrink-0"
                        style={{
                          backgroundColor: DIGITIZATION_COLORS[nearby.digitizationLevel],
                        }}
                      />
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-gray-500">
                          {nearby.city} · {Math.round(distance)} km
                        </div>
                      </div>
                    </div>
                  </button>
